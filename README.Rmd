...
* **Hierarchical Clustering** `hclust.sevt(object, distance, k, method, ignore, limit, scope)`
```{r}
...
mod5
```
* **K-Means Clustering** `kmeans.sevt(object, k, algorithm, ignore, limit, scope, nstart)`
```{r}
